    ...
    def do_GET(self):
        try:
            ...
            if self.path.endswith("/edit"):
                #Inputs restaurant's ID into url
                restaurantIDPath = self.path.split("/")[2]
                #Query restaurant's by ID within URL path and returns first one found
                myRestaurantQuery = session.query(Restaurant).filter_by(
                    id=restaurantIDPath).one()
                #If a value for restaurant.id is found within myRestaurantQuery, will return an html page.
                if myRestaurantQuery:
                    ...
                    output = "<html><body>"
                    output += "<h1>"
                    #Uses ID query to display restaurant name 
                    output += myRestaurantQuery.name
                    output += "</h1>"
                    #Creates form for inputing restaurants name.
                    output += "<form method='POST' enctype='multipart/form-data' action = '/restaurants/%s/edit' >" % restaurantIDPath
                    #Passes restaurant's name into newRestaurantName. Prefills form with restaurant's current name
            ...
            if self.path.endswith("/delete"):
                ...
                if myRestaurantQuery:
                    ...
                    self.end_headers()
                    output = "<html><body>"
                    output += "<h1>This will permanently delete %s." % myRestaurantQuery.name
                    #Creates form for deleting restaurants name.
                    output += "<form method='POST' enctype='multipart/form-data' action = '/restaurants/%s/delete' >" % restaurantIDPath
                    #Passes restaurant's name into deleteRestaurantName
                    output += "<input name = 'deleteRestaurantName' type = 'submit' value = 'Delete'>" 
                    output += "</form>"
                    output += "</body></html>"
            ...
            if self.path.endswith("/restaurants"):
                ...
                for restaurant in restaurants:
                    ...
                    # Objective 4 -- Uses for loop to find ID in restaurants which is a query. 
